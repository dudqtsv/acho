...
if ($id == 0) {
    ...
    // editar
    if ($nome_arquivo == "") {
        $sql = "UPDATE usuarios SET nomeUsuario = ?, emailUsuario = ?, dataNascimentoUsuario = ?, cpfUsuario = ?, usernameUsuario = ?, senhaUsuario = ? WHERE idUsuario = ?";
        $comando = mysqli_prepare($conexao, $sql);
        mysqli_stmt_bind_param($comando, 'ssssssi', $nome, $email, $data_nascimento, $cpf, $username, $senha, $id);
    } else {
        $caminho_temporario = $_FILES['foto']['tmp_name'];
        $extensao = pathinfo($nome_arquivo, PATHINFO_EXTENSION);
        $novo_nome = uniqid() . "." . $extensao;
        ...
        $comando = mysqli_prepare($conexao, $sql);
        mysqli_stmt_bind_param($comando, 'sssssisi', $nome, $email, $data_nascimento, $cpf, $username, $senha, $municipio, $novo_nome, $id);
    }
    mysqli_stmt_execute($comando);
